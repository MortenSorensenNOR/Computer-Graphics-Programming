...
void* fragment_shader_thread_func(void* arg_ptr) {
    ...
    for (int y = thread_arg->batch_start_y; y < thread_arg->batch_end_y; ++y) {
        for (int x = 0; x < thread_arg->s_width; x++) {
            ...
            unsigned long long diffuse_texture_index = (int)(thread_arg->uv[i].x * thread_arg->textures->width) + 
                                                       (int)(thread_arg->uv[i].y * thread_arg->textures->height) * thread_arg->textures->width;
            diffuse_texture_index = fmin(diffuse_texture_index, thread_arg->textures->width * thread_arg->textures->height);

            glm::vec3 diffuse_base = thread_arg->textures->data[diffuse_texture_index];
            ...
            glm::vec3 norm = glm::normalize(thread_arg->norm[i]);
            glm::vec3 lightDir = thread_arg->light->pos - thread_arg->frag[i];
...
int fragment_shader(fragment_shader_input_t* in, fragment_shader_output_t* out) {
    ...
    for (int y = 0; y < in->s_height; ++y) {
        for (int x = 0; x < in->s_width; ++x) {
            ...
            // glm::vec3 diffuse_base = in->textures->diffuse[diffuse_texture_index];
            glm::vec3 diffuse_base = in->textures->diffuse[diffuse_texture_index];
            glm::vec3 metalic_roughness = in->textures->specular[specular_texture_index];
            ...
            glm::vec3 norm = vec3_normalize(&in->norm[i]);
            glm::vec3 lightDir = vec3_sub(&in->light->pos, &in->frag[i]);
            ...
            // TODO: Fix specular
            // glm::vec3 viewDir = vec3_sub(&in->cam_pos, &in->frag[i]);
            ...
            // glm::vec3 reflectDir = vec3_mul(&norm, 2.0f * norm_light_proj);
            ...
            glm::vec3 result = vec3_add(&ambient, &diffuse);
